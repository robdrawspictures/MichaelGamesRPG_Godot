...
func add_item(item: ItemData, count: int = 1) -> bool:
	...
	# If item doesn't exist, check available invetory space
	# and add new item if an empty slot exists
	for i in slots.size():
		if slots[i] == null:
...
func item_to_save(slot: SlotData) -> Dictionary:
	...
	if slot != null:
		...
		if slot.item_data != null:
			result.item = slot.item_data.resource_path # Grabs the data from the actual .tres file for the item
	return result
